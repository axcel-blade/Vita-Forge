...
### Changed
- **Breaking:** A user's profile now stores multiple resumes and cover letters instead of one of each. `StoredProfileRecord` (`src/repositories/data-store.ts`) replaces the singular `resume`/`coverLetter` fields with `resumes`/`coverLetters` arrays of `{ id, title, data, createdAt, updatedAt }`; `UpsertProfileDto` validates each array entry via the new `ProfileDocumentDto` (`src/users/dto/profile-document.dto.ts`). `POST /users/profile` still writes the whole profile in one call — the frontend reads/splices the array client-side (`frontend/src/services/documents.ts`)
...
### Added
- `database/seed.ts` now seeds multiple sample resumes and cover letters (matching the `resumes`/`coverLetters` array shape) instead of a single resume payload
